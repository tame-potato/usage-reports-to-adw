...
#
# Requires oracledb and oci python package
...
def set_parser_arguments():
    ...
    parser.add_argument('-ds', default="", dest='dsecret', help='ADB Secret Id')
    ...
    parser.add_argument('-ws', default="", dest='wallet_secret', help='Wallet Secret Id')
    ...
    if not (result.duser and result.dsecret and result.dname and result.csv_location):
        parser.print_help()
...
def main_process():
    ...
    dpass = get_secret_password(config, signer, cmd.proxy, cmd.dsecret)

    wallet_password = ""
    if cmd.wallet_secret:
        wallet_password = get_secret_password(config, signer, cmd.proxy, cmd.wallet_secret)
    ...
    try:
        ...
        with oracledb.connect(user=cmd.duser, password=dpass, dsn=cmd.dname, config_dir=cmd.wallet_location, wallet_location=cmd.wallet_location, wallet_password=wallet_password) as connection:
